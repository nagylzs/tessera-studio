...
- `lib/widgets/tessera_logo.dart`: the icon drawn with a `CustomPainter`
  ...
  name… 42%") while the argument loads. `SchemaPage` is the screen after
  loading and still a placeholder (name, format, size, close button;
  Android back closes the file through `PopScope`); the workbench comes
  after it.
